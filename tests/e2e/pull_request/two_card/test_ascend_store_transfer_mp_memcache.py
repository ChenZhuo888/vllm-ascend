...
"""Exercise AscendStore subprocess Memcache transfers."""
...
PROMPT_LENGTH = 1536  # One hybrid KV transfer block at TP2.
PROMPT_SALTS = (
...
def test_ascend_store_memcache_roundtrip(monkeypatch) -> None:
    ...
    with SingleNodeMemcacheManager(config, f"ascend-store-transfer-{mode}-{uuid.uuid4().hex}") as manager:
        ...
        with VllmRunner(
            ...
            for case_id, salt in enumerate(PROMPT_SALTS):
                ...
                prompt = TokensPrompt(
                    prompt_token_ids=[1 + (salt[index % len(salt)] + index) % 1023 for index in range(PROMPT_LENGTH)]
                )
                ...
                cold_tokens = cold_output.outputs[0].token_ids
                loaded_tokens = loaded_output.outputs[0].token_ids
                assert loaded_tokens == cold_tokens, (
                    f"Memcache load changed greedy output: {case}, cached_tokens={cached_tokens}, "
                    f"cold_tokens={cold_tokens}, loaded_tokens={loaded_tokens}"
                )
