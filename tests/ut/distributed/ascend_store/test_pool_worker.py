...
class TestKVPoolWorkerRegisterAndTransfer(unittest.TestCase):
    ...
    def test_lookup_hash_modes_preserve_terminal_key_and_hit(self):
        """FULL and SUFFIX differ only on the wire, not in lookup semantics."""
        worker = self._make_worker()
        ...
        cases = [
            (None, LookupHashMode.FULL, request.block_hashes),
            ("suffix", LookupHashMode.SUFFIX, request.block_hashes[2:]),
        ]
        module = "vllm_ascend.distributed.kv_transfer.kv_pool.ascend_store.pool_scheduler"
        lookup_results = []
        for configured_mode, mode, expected_payload in cases:
            with self.subTest(mode=mode.value):
                ...
                lookup_results.append(scheduler.get_num_new_matched_tokens(request, 32))
        ...
        self.assertEqual(lookup_results, [(32, False), (32, False)])
