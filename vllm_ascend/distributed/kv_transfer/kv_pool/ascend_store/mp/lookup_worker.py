...
class LookupKVPoolWorker(KVPoolWorker):
    """Reuse KVPoolWorker Lookup without initializing process-local distributed state."""
    ...
    def __init__(
        ...
        self._registered_rank = parallel_config.rank if rank is None else rank
        self._store_is_external = store is not None
        self.m_store: LookupStore = store if store is not None else _MissingLookupStore()
        super().__init__(vllm_config, use_layerwise=False, kv_cache_config=kv_cache_config)
        del self._registered_rank
    ...
    def _init_backend(self, _parallel_config, _extra_config) -> None:
        """Keep the Lookup store supplied to or later bound by the service manager."""
        pass
